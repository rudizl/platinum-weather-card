...
export class PlatinumWeatherCard extends LitElement {
  ...
  private _renderChartSection(): TemplateResult {
    ...
    if (typeof (window as any)._pwcActiveTip !== 'number') (window as any)._pwcActiveTip = -1;
    const activeDay: number = (window as any)._pwcActiveTip;
    (window as any)._pwcTipClick = (el: HTMLElement, idx: number, ev: Event) => {
      ev.stopPropagation();
      const section = el.closest('.pwc-chart-wrap') as HTMLElement;
      if (!section) return;
      const cur: number = (window as any)._pwcActiveTip;
